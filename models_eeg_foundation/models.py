...
class MultiViewGenerator:
    ...
    def generate(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        return {
            "global_a": x,
            "global_b": x + torch.randn_like(x) * 0.1,
            "local_time_1": lt1, "local_time_2": lt2,
            "local_freq_1": self._bandpass_fft(x, 4.0, 12.0),
            "local_freq_2": self._bandpass_fft(x, 12.0, 30.0),
        }
...
class _BaseDualCDModel(nn.Module):
    ...
    def __init__(self, config, d_model, lambda_intra, dino_out_dim,
        ...
        self.multi_view = MultiViewGenerator(seq_len=config.seq_len, sfreq=sfreq)
