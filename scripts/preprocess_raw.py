...
def main() -> None:
    ...
    logger.info("config=%s | %s/%s | variant=%s", args.config, subj, sess, cfg.get("variant"))
    logger.info("data.bdf=%s", data_bdf)

    # TODO(Task 2): 调用 src.preprocessing.shu_preprocess.preprocess_one_session，
    # 校验形状 == cfg['expect_shape']，保存到 data/<variant>/...，并写 meta.json。
    raise NotImplementedError(
        "preprocess_raw 待实现。先跑 check_raw_bdf.py，再按 docs/PREPROCESSING_SPEC.md 实现。"
    )
