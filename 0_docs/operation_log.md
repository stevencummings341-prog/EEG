...
| 2026-08-13 00:30:00 | CHMOD | `/share/home/Zihang`（项目外，用户批准） | `700 -> 711`（`chmod o+x`）：让同集群另一账号能穿过家目录读 `MI/` 下的 run 产物；未给 r 位故仍无法列目录。 |
| 2026-08-13 01:11:00 | CHMOD | `/share/home/Zihang`（项目外，用户要求收回） | `711 -> 700`（`chmod o-x`）：已收回穿过权限，家目录重新仅本人可进。 |
